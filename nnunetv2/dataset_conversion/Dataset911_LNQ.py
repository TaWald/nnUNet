...
def total_segmentator_dir(case_dir, output_dir):
...
def find_mutually_exclusive_classes_with_total_segmentator(total_segmentator_dir: Path, groundtruth_dir: Path, out_path: Path):
    ...
    if out_path.exists():
        ...
        for ts, gt in zip(all_total_segmentator_files, all_groundtruth_files):
            assert ts.split("-")[2] == gt.split("-")[2], "Total segmentator and groundtruth files do not match!"
        ...
        for ts, gt in tqdm(zip(all_total_segmentator_files, all_groundtruth_files)):
            ...
            for i in range(105):
                ...
                all_results[i].append(float(np.sum(np.logical_and(ts_foreground_mask, lymphnode_data), dtype=float) / float(n_lymphnode)))
        ...
        non_zero_mean = {}
        for k, v in all_results.items():
            non_zero_mean[k] = float(np.mean(np.mean([x for x in v if x != 0]) if len([x for x in v if x != 0]) != 0 else 0))
...
def create_nnunet_dataset(train_image_path: Path, groundtruth_image_path: Path, output_path: Path, dataset_name: str):
    ...
    train_path = output_path / f"DatasetXXX_LNQ_{dataset_name}" / "imagesTr"
    label_path =  output_path / f"DatasetXXX_LNQ_{dataset_name}" / "labelsTr"
...
def create_groundtruth_given_totalsegmentator(total_segmentator_dir: Path, total_segmentator_background_class_ids: Sequence[int], groundtruth_dir: Path, output_dir: Path, overwrite=False):
    ...
    for ts, gt in tqdm(zip(all_total_segmentator_files, all_groundtruth_files)):
        ...
        final_groundtruth = np.full_like(lnq_data, fill_value=1)  # 0 will be background, 1 is ignore label and 2 is foreground (lymph node)
        ...
        mask = test_multidim_isin(ts_data, total_segmentator_background_class_ids)  # This is slow but running it once is enough, so who cares

        # assert np.all(mask == mask_1), "Masks are not equal!"

        final_groundtruth = np.where(mask, 0, final_groundtruth)
        final_groundtruth = np.where(lnq_data != 0, 2, final_groundtruth)

        final_groundtruth = final_groundtruth.reshape(lnq_data.shape)

        lnq_binary = np.where(lnq_data != 0, 1, 0)
        dilated_lnq_binary = morphology.binary_dilation(lnq_binary, iterations=1)  # Square connectivity equal to one
        background_lnq = dilated_lnq_binary - lnq_binary

        final_groundtruth = np.where(background_lnq != 0, 0, final_groundtruth)
...
def main():
    ...
    path_to_data = Path("/home/tassilowald/Data/Datasets/LNQ/")
    ...
    train_im_labels = [get_im_and_label_from_id(id, train_dir, False) for id in train_ids]
    ...
    remaining_cases, remaining_labels = filter_non_same_size_cases(only_train_images, only_train_labels)
    convert(remaining_cases, temp_in_path)
    convert(remaining_labels, temp_lbl_path)
    total_segmentator_dir(temp_in_path, temp_out_path)

    mean_res, non_zero_mean, all_results = find_mutually_exclusive_classes_with_total_segmentator(temp_out_path, temp_lbl_path, meta_info_path)

    background_classes_no_aorta = {int(k) for k, v in non_zero_mean.items() if not (int(k) in [0, 7])}  # 0 is background, so we do not want that and do not want classes that overlap with lymphnodes.
    background_classes_non_zero = {int(k) for k, v in mean_res.items() if (v == 0 and k != 0)}  # 0 is background, so we do not want that and do not want classes that overlap with lymphnodes.
    background_classes_medium_overlap = {int(k) for k, v in non_zero_mean.items() if (v < 0.1 and k != 0)}  # 0 is background, so we do not want that and do not want classes that overlap with lymphnodes.
    background_classes_low_overlap = {int(k) for k, v in non_zero_mean.items() if (v < 0.01 and k != 0)}  # 0 is background, so we do not want that and do not want classes that overlap with lymphnodes.

    # Create different groundtruths
    create_groundtruth_given_totalsegmentator(temp_out_path, background_classes_no_aorta, temp_lbl_path, out_dir_aorta)
    create_groundtruth_given_totalsegmentator(temp_out_path, background_classes_non_zero, temp_lbl_path, out_dir_non_zero)
    create_groundtruth_given_totalsegmentator(temp_out_path, background_classes_low_overlap, temp_lbl_path, out_dir_low_overlap)
    create_groundtruth_given_totalsegmentator(temp_out_path, background_classes_medium_overlap, temp_lbl_path, out_dir_medium_overlap)

    # Now we create the nnUNet compatible datasets
    create_nnunet_dataset(train_image_path=temp_in_path, groundtruth_image_path=out_dir_aorta, output_path=path_to_data / "nnunet",  dataset_name="aorta-not-background")
    create_nnunet_dataset(train_image_path=temp_in_path, groundtruth_image_path=out_dir_low_overlap, output_path=path_to_data / "nnunet",  dataset_name="low-overlap-not-background")
    create_nnunet_dataset(train_image_path=temp_in_path, groundtruth_image_path=out_dir_medium_overlap, output_path=path_to_data / "nnunet",  dataset_name="medium-overlap-not-background")
    create_nnunet_dataset(train_image_path=temp_in_path, groundtruth_image_path=out_dir_non_zero, output_path=path_to_data / "nnunet",  dataset_name="non-zero-overlap-not-background")
